...
async function fetchFromBackend(endpoint: string, params: Record<string,string>) {
...
(document.getElementById('syncApi') as HTMLButtonElement).onclick = async () => {
  ...
  try {
    ...
    state.vat = await fetchFromBackend($('epVat').value, { company_id: c, period_start: $('fs').value, period_end: $('pe').value });
    state.tax = await fetchFromBackend($('epTax').value, { company_id: c, period_start: $('fs').value, period_end: $('pe').value });
    state.gl = await fetchFromBackend($('epGl').value, { company_id: c, period_start: $('fs').value, period_end: $('pe').value });
